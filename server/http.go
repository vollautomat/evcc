...
func (s *HTTPd) RegisterSiteHandlers(site site.API, cache *util.Cache) {
	...
	routes := map[string]route{
		...
		"loadpoints":              {[]string{"GET"}, "/config/loadpoints", loadpointsHandler},
		"newloadpoint":            {[]string{"POST", "OPTIONS"}, "/config/loadpoints", newLoadpointHandler},
		"updateloadpoint":         {[]string{"PUT", "OPTIONS"}, "/config/loadpoints/{id:[0-9.]+}", updateLoadpointHandler},
		"deleteloadpoint":         {[]string{"DELETE", "OPTIONS"}, "/config/loadpoints/{id:[0-9.]+}", deleteLoadpointHandler},
