...
// setup messaging
func configureMessengers(conf messagingConfig, valueChan chan util.Param, cache *util.Cache) error {
	hub, err := push.NewHub(conf.Events)

	// always do this to setup channels and handler
	push.Prepare(hub, valueChan, cache)
